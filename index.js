...
var desc = require('./param_desc.json');
// consts
        ...
        if(err)
            ...
            pdf.convert(config, function(err, result) {
                if(err)    
					...
					result.toFile("./"+fileName+".pdf", function(err) {
						if(err)    
							console.log("err:" + err);
						else
  ...
  html += "<style>";
    ...
    if(typeof(swaggerJSON.definitions) !=='undefined')
    {
        ...
        for(var dfn in swaggerJSON.definitions) {
            ...
            html += "<a name='" + dfn + "'><h3>1." +sub1Counter + ". " + dfn + "</h3></a>";
            html += "<hr />";
    ...
    html += "</div></div>";
    html += "<h2>3. code值返回说明:</h2>";
    html += '<div>'; // path start
    html += "<table class='table-margin'>";
    for(var i in desc['code']) {
      html += "    <tr>";
      html += "           <td class='td-alignment-small'><b>"+i+":</b></td>";
      html += "           <td class='td-alignment-std'>" + desc['code'][i] + "</td>";
      html += "    </tr>";
    }
    html += "</table></div>";
...
function renderSchemaItems(schemaItems, swaggerDefinitions){
    ...
    if(typeof schemaItems["$ref"] !== "undefined"){
        ...
        html += "See <a href='#" + dfn + "'><b>" + dfn + "</b></a> in the <b>Definitions</b> section.";
        html += "<br />";
        html += "<br />";
...
function renderDefinition(minimal, dfn, swaggerJSONdefinitions){
    ...
    for(var dfnProps in swaggerJSONdefinitions[dfn].properties){
        ...
        html += "   <tr style='" + rowStyle + "'>";
        html += "       <td style='width:30%;'>" + ccc(dfnProps) + "</td>";
        if(swaggerJSONdefinitions[dfn].properties[dfnProps] != null){
            ...
            if(!minimal){
              ...
              html += ((typeof desc['field'][dfnProps] !== "undefined") ? desc['field'][dfnProps]: " ");
              ...
              if(typeof swaggerJSONdefinitions[dfn].properties[dfnProps]["$ref"] !== "undefined"){
                    ...
                    html += "See <a href='#" + subdfn + "'><b>" + subdfn + "</b></a> in the <b>Definitions</b> section.";
                }
        ...
        var isRequired = true;
      if(swaggerJSONdefinitions[dfn].properties[dfnProps] != null){
        if (swaggerJSONdefinitions[dfn].properties[dfnProps].hasOwnProperty("required") && swaggerJSONdefinitions[dfn].properties[dfnProps].required === false){
          isRequired = false;
        }
      }
        ...
        // }
        html += "       <td style='width:30%;'>" + (isRequired===false?"no":"yes") + "</td>";
        html += "   </tr>";
...
function renderSecurityDefinitions(securityDefinitions){
    ...
    for(var sec in securityDefinitions) {
            ...
            html += "<tr>";
            var defType = ( typeof(securityDefinitions[sec].type) !== "undefined" ? " (" + securityDefinitions[sec].type + ")" : "" );
            html += "<td colspan='2'><a name='"+deffinameType+"'><h3>"+main2Counter +"." + sub2Counter +". "  + sec +
              defType + "</h3></a>" + "</td>";
            ...
            if(typeof(securityDefinitions[sec].name) !== "undefined"){
                ...
                html += "<td>" +( typeof(securityDefinitions[sec].name) !== "undefined" ? ccc(securityDefinitions[sec].name) : "" ) + "</td>";
                html += "</tr>";       
            }
